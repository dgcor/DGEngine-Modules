module dgengine.parser.rectangle;
...
namespace Parser
{
	...
	std::shared_ptr<Rectangle> getRectangleObj(Game& game, const Value& elem)
	{
		...
		rectangle->setFillColor(getColorKey(elem, "color", sf::StaticColor::White()));
		rectangle->setOutlineColor(getColorKey(elem, "outlineColor", sf::StaticColor::White()));
