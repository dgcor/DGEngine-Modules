module dgengine.parser.animation;

import dgengine.gameutils;
import dgengine.panel;
import dgengine.texturepacks.simpletexturepack;
...
namespace Parser
{
	...
	std::shared_ptr<Animation> getAnimationObj(Game& game, const Value& elem)
	{
		...
		if (refresh == sf::microseconds(0))
		{
			refresh = sf::milliseconds(50);
		}
		animation->setFrameTime(getTimeKey(elem, "refresh", refresh));
		animation->setColor(getColorKey(elem, "color", sf::StaticColor::White()));

		auto outline = getColorKey(elem, "outline", sf::StaticColor::Transparent());
		auto outlineIgnore = getColorKey(elem, "outlineIgnore", sf::StaticColor::Transparent());
