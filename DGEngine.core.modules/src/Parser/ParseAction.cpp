module dgengine.parser.action;

import dgengine.actions;
import dgengine.fileutils;
import dgengine.hooks;
import dgengine.json.utils;
import dgengine.parser.condition;
	...
	std::shared_ptr<Action> parseActionElem(Game& game, const Value& elem)
	{
		...
		case str2int16("button.setColor"):
		{
			return std::make_shared<ActButtonSetColor>(
				getStringViewKey(elem, "id"),
				getColorKey(elem, "color", sf::StaticColor::White()));
		}
		...
		case str2int16("cursor.setOutline"):
		{
			return std::make_shared<ActCursorSetOutline>(
				getColorKey(elem, "outline", sf::StaticColor::Transparent()),
				getColorKey(elem, "ignore", sf::StaticColor::Transparent()));
		}
		...
		{
			auto action = std::make_shared<ActGameFade>(
				getColorKey(elem, "color", sf::StaticColor::Transparent()),
				true,
