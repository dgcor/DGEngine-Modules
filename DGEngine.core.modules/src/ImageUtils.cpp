...
namespace ImageUtils
{
	...
	{
		if (transparencyMask != sf::StaticColor::Transparent())
		{
		...
		const std::shared_ptr<Palette>& pal)
	{
		...
		sf::Image img;
		img.create(imgWidth, imgHeight, sf::StaticColor::Transparent());
		...
		const std::string_view fileNameCharMap, const std::shared_ptr<Palette>& pal)
	{
		...
		sf::Image img;
		img.create(firstImgSize.x * 16, firstImgSize.y * 16, sf::StaticColor::Transparent());
