...
export
{
	...
	public:
		...
		Helper2D<Vector2D, T&, size_t> operator[] (size_t x) noexcept
		{
			return Helper2D<Vector2D, T&, size_t>(*this, x, get);
		}

		Helper2D<const Vector2D, const T&, size_t> operator[] (size_t x) const noexcept
		{
			return Helper2D<const Vector2D, const T&, size_t>(*this, x, get);
		}
